...
our ($DEBUGGER, @RULES) = ($DI::DEBUGGER, qw/classes all/);
...
sub addComputer {
	...
	if(!exists $self->{'classes'}->{$classId}) {
		warn $DEBUGGER->make_error('ERROR', $self, "Tried add computer to class:<$classId> but it seems not exists.");
		return undef;
	};
	my $mac;
	unless($mac = $self->parseMac($plainMac)) {
		die $DEBUGGER->make_error('FATAL_ERROR', $self, "Tried to add computer with wrong mac:<$plainMac>.");
	};
	if($self->macExists($mac)) {
		die $DEBUGGER->make_error('FATAL_ERROR', $self, "Tried to add computer with mac that already in DB:<$mac>");
	};
	if(defined $ip && length($ip) && !defined $self->parseIp($ip)) {
		die $DEBUGGER->make_error('FATAL_ERROR', $self, "Tried to add computer with wrong IP.<$ip>");
	};
	...
	if(!defined $computer) {
		...
		if(exists $opts{'mac'} && $opts{'mac'} != $computer->{'mac'}) {
			if($self->macExists($opts{'mac'})) {
				die $DEBUGGER->make_error('FATAL_ERROR', $self, "Tried to update computer with mac that already in DB:<$opts{'mac'}>");
			};
...
sub addIfNotExists {
	...
	if(!defined $IAD::Config::add_new_to_group || $self->macExists($mac)) {
		...
		$DEBUGGER->print_message([@RULES], $self, "Adding new computer automaticaly.");
